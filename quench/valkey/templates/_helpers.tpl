...
{{- define "valkey.primary.bootstrapHost" -}}
{{- $primary := include "valkey.primary.fullname" . -}}
...
{{- define "valkey.defaultAntiAffinity" -}}
{{- $ := .root -}}
podAntiAffinity:
...
Sentinel and writes a runtime valkey.conf (base + replicaof) into an emptyDir the
shell-free valkey container then loads. Call: (dict "root" . "isPrimary" bool).
*/}}
{{- define "valkey.discoverInitContainer" -}}
{{- $ := .root -}}
- name: discover-master
  image: {{ include "valkey.busyboxImage" $ }}
  ...
  env:
    ...
    - name: SENTINEL_SVC
      value: {{ include "valkey.sentinel.fullname" $ | quote }}
    - name: SENTINEL_PORT
      value: {{ $.Values.sentinel.port | quote }}
    - name: MASTER_SET
      value: {{ $.Values.sentinel.masterSet | quote }}
    - name: BOOTSTRAP_HOST
      value: {{ include "valkey.primary.bootstrapHost" $ | quote }}
    - name: IS_PRIMARY
