...
- **coolify-app** — the PHP/Laravel core: php-fpm + Quenchworks nginx (UI + API on
  `:8080`, health at `GET /healthcheck`) plus Horizon (queue workers) and the Laravel
...
- **Honesty note:** the helper/proxy tier on those managed hosts is **privileged by
  design** — it drives the host's Docker daemon. That privilege lives on the external
